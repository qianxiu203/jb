...
def main() -> None:
    ...
    # 修改：默认间歇时间改为 20 分钟 (1200 秒)
    parser.add_argument("--sleep-min", type=int, default=1200, help="循环模式最短等待秒数 (默认 1200 秒 = 20 分钟)")
    parser.add_argument(
        "--sleep-max", type=int, default=1200, help="循环模式最长等待秒数 (默认 1200 秒 = 20 分钟)"
    )
    ...
    # 优化：启动时显示间歇时间配置
    print(f"[Info] 间歇时间配置: {sleep_min/60:.1f} - {sleep_max/60:.1f} 分钟 ({sleep_min}-{sleep_max} 秒)")
    ...
    while True:
        ...
        print(f"[*] 休息 {wait_time} 秒 ({minutes} 分 {seconds} 秒) 后继续...")
